...
texti03 = "&0 <strong>Comandi:</strong>\n" \
          ...
          "/es - Traduzione in Spagnolo 🇪🇸 \n" \
          "/de - Traduzione in Tedesco 🇧🇪 \n" \
          "/switch - Scambia la lingua &1\n"\
          "/help - Lista dei comandi &2\n"
...
def mostra_start(chat_id,name):
    testo = texti02.replace('&0', name.encode('utf-8'))
    testo = testo.replace('&1', emoji.emojize(':sunglasses:', use_aliases=True).encode('utf-8'))
    testo = testo.replace('&2', emoji.emojize(':gear:', use_aliases=True).encode('utf-8'))
    testo = testo.replace('&3', emoji.emojize(':v:', use_aliases=True).encode('utf-8'))
    testo = testo.replace('&4', emoji.emojize(':arrows_counterclockwise:', use_aliases=True).encode('utf-8'))
    testo = testo.replace('&5', emoji.emojize(':book:', use_aliases=True).encode('utf-8'))

    bot.sendMessage(chat_id, testo, parse_mode='HTML')


def mostra_info(chat_id):
    testo = texti03.replace('&0', emoji.emojize(':gear:', use_aliases=True).encode('utf-8'))
    testo = testo.replace('&1', emoji.emojize(':arrows_counterclockwise:', use_aliases=True).encode('utf-8'))
    testo = testo.replace('&2', emoji.emojize(':book:', use_aliases=True).encode('utf-8'))
    bot.sendMessage(chat_id, testo, parse_mode='HTML')
